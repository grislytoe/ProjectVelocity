...
- Floor, wall contact, wall slide and wall jump restore extra jump and Dash. Wall sliding retains downward gravity with a speed cap; contact probes preserve wall sliding without requiring the player to push into a wall. Wall jump pushes upward/outward and locks horizontal steering for nine ticks. Surface contacts are explicit data so later surface modifiers can be applied before simulation.
