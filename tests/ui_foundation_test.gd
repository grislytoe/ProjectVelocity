...
func run() -> void:
	...
	for locale: String in ["en", "ru"]:
		...
		check(not find_button("UI_VIDEO").disabled, "M12 settings enabled")
		for category: String in ["VIDEO", "AUDIO", "ACCESSIBILITY"]:
			await activate("UI_" + category)
			await capture("m12-" + category.to_lower() + "-" + locale)
			await action("ui_focus_next", true)
			check(root.gui_get_focus_owner() != null, "Settings controller focus")
			await action("ui_cancel", true)
