...
The validator imports assets, parses all bootstrap/test scripts, runs behavior tests, boots the actual main scene through a physics tick and checks Git whitespace. With -ExportWindows it exports and boots the Windows staging executable. Logs go to ignored builds/validation.

## Build identity

Version 0.0.1-dev and build number 1 live in core/build/build_info.gd. Keep project.godot's application version synchronized; tests enforce this. OS.is_debug_build() is the authoritative development flag. The staging export adds the staging feature; its label is STAGING. Release-mode engine binaries disable development debug logging; release exports require developer review and are not part of M0.
